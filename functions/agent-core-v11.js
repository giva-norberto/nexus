...
function hasListaLarContext(prompt, history) {
  ...
  return /\blistalar\b|\blista lar\b/.test(normalize(`${prompt} ${recent}`));
}
...
function enforceSourcePriority(planResult, prompt, history) {
  ...
  if (isOperationalDataQuestion(prompt, history)) {
    const existing = plan.tools.find((tool) => tool?.name === 'listalar_analytics');
    const analyticsCall = existing || { name: 'listalar_analytics', args: {} };
    analyticsCall.args = { ...(analyticsCall.args || {}), project: 'listalar', query: prompt };
    plan.tools = [analyticsCall, ...plan.tools.filter((tool) => tool?.name !== 'listalar_analytics' && tool?.name !== 'github_investigate')];
    plan.project = 'listalar';
    plan.needsClarification = false;
    plan.clarification = '';
  }
  plan.tools = plan.tools.filter((tool) => ['memory_search','github_investigate','listalar_analytics'].includes(tool?.name)).slice(0, MAX_TOOL_CALLS);
  return plan;
}
  ...
  for (const candidate of candidates) {
    try {
      ...
      for (let i = 0; i < lines.length; i += 1) {
        ...
        if (terms.some((term) => term.length >= 4 && line.includes(term))) hits.push(i);
      }
...
async function toolListaLarAnalytics(args, userPrompt) {
  ...
  for (const family of families.docs) {
    ...
    for (const gastoDoc of gastos.docs) {
      ...
      const purchaseMs = dateMs(gasto.dataCompraMs || gasto.dataCompra || gasto.criadoEm);
      if (cutoff && Number.isFinite(purchaseMs) && purchaseMs < cutoff) continue;
      purchases += 1;
      totalSpent += toNumber(gasto.valorTotal);
      ...
      for (const itemDoc of itemSnap.docs) {
        ...
        const unitPrice = Math.max(0, toNumber(item.precoUnitario));
        ...
        if (current.history.length < 60) current.history.push({
          dateMs: Number.isFinite(purchaseMs) ? purchaseMs : null,
          date: Number.isFinite(purchaseMs) ? new Date(purchaseMs).toISOString() : null,
  ...
  const products = [...productsMap.values()].map((product) => {
    const history = product.history.filter((entry) => entry.unitPrice > 0).sort((a, b) => (a.dateMs ?? Number.MAX_SAFE_INTEGER) - (b.dateMs ?? Number.MAX_SAFE_INTEGER));
    ...
    return {
      name: product.name,
      totalSpent: money(product.totalSpent),
      quantity: Math.round(product.quantity * 1000) / 1000,
      occurrences: product.occurrences,
      avgUnitPrice: product.quantity > 0 ? money(product.totalSpent / product.quantity) : 0,
      firstUnitPrice: first?.unitPrice || 0,
      lastUnitPrice: last?.unitPrice || 0,
      priceChange: change,
      priceChangePct: changePct,
      minUnitPrice: history.length ? Math.min(...history.map((entry) => entry.unitPrice)) : 0,
      maxUnitPrice: history.length ? Math.max(...history.map((entry) => entry.unitPrice)) : 0,
      history: history.slice(-20).map(({ dateMs: _, ...entry }) => entry)
    };
  });
  ...
  return {
    ...
    matchingProducts: matching.slice(0, 12),
    topSpend: [...products].sort((a, b) => b.totalSpent - a.totalSpent).slice(0, 10),
    topFrequency: [...products].sort((a, b) => b.occurrences - a.occurrences).slice(0, 10),
    priceIncreases: products.filter((product) => product.occurrences >= 2 && product.priceChange > 0).sort((a, b) => b.priceChangePct - a.priceChangePct).slice(0, 10),
    priceDecreases: products.filter((product) => product.occurrences >= 2 && product.priceChange < 0).sort((a, b) => a.priceChangePct - b.priceChangePct).slice(0, 10),
    mostExpensive: [...products].sort((a, b) => b.maxUnitPrice - a.maxUnitPrice).slice(0, 10),
...
async function plan(prompt, history) {
  const plannerSystem = [
    'Você é o planejador do Nexus Agent Core.',
    'Retorne APENAS JSON válido.',
    'Ferramentas permitidas: memory_search, github_investigate, listalar_analytics.',
    'REGRA DE AUTORIDADE: perguntas sobre fatos operacionais devem consultar a fonte de dados operacional, não o código-fonte.',
    'Para ListaLar: compras, produtos, gastos, preços, aumento ou queda, frequência, histórico, mercado e estabelecimento => listalar_analytics.',
    'GitHub só é fonte prioritária quando a pergunta é sobre código, bug, arquitetura, implementação, arquivo, função ou estado do repositório.',
    'Use memory_search para regras, decisões, preferências e contexto persistente.',
    'Para continuação, use o histórico recente para recuperar projeto, produto e assunto.',
    'Máximo 3 chamadas.',
  ...
  const parsed = parseJson(content) || { objective: prompt, project: '', entities: [], tools: [], needsClarification: false, clarification: '' };
  return enforceSourcePriority(parsed, prompt, history);
}
...
exports.askNexusAgent = onCall(
  ...
  async (request) => {
    ...
    for (const call of planResult.tools) {
      try {
        evidence.push(await executeTool(call, prompt));
      } catch (error) {
        console.error('Agent tool failed', call?.name, error);
        evidence.push({ tool: call?.name || 'unknown', ok: false, error: String(error?.message || error) });
      }
    }
    ...
    const answerSystem = [
      ...
      'Responda exatamente ao que foi perguntado e use a fonte operacional mais autoritativa disponível.',
      'Não substitua dados do Firestore por explicação de código quando a pergunta for sobre valores, compras, produtos ou preços reais.',
      'Use somente as evidências fornecidas para afirmar fatos.',
      'Se a ferramenta operacional retornou dados, responda com esses dados antes de qualquer explicação técnica.',
      'Em preço, diferencie preço unitário, total gasto e quantidade.',
      'Em evolução de preço, compare primeiro e último preço cronologicamente e informe variação percentual.',
      'Em investigação de código, cite arquivo e linhas quando disponíveis.',
      'Esta função é somente leitura e não pode afirmar que alterou, mergeou, publicou ou gravou dados.',
      'Evite relatório padrão; responda especificamente à pergunta.'
    ].join(' ');
    ...
    return {
      ...
      plan: {
        ...
        tools: planResult.tools.map((tool) => tool.name)
      },
