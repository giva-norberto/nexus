...
async function toolListaLarAnalytics(args, userPrompt) {
  const db = getFirestore(getListaLarApp()); const productHint = normalize(args?.product || args?.entity || '');
  const days = Math.max(0, Math.min(3650, Number(args?.days || 0))); const cutoff = days ? Date.now() - days * 86400000 : null;
  const families = await db.collection('familias').limit(MAX_FAMILIES).get(); const productsMap = new Map(); let purchases = 0, itemRows = 0, totalSpent = 0;
  for (const family of families.docs) {
    ...
    for (const gastoDoc of gastos.docs) {
      ...
      if (cutoff && Number.isFinite(purchaseMs) && purchaseMs < cutoff) continue; purchases += 1; totalSpent += toNumber(gasto.valorTotal);
      const itemSnap = await gastoDoc.ref.collection('itens').limit(MAX_ITEMS - itemRows).get();
      for (const itemDoc of itemSnap.docs) {
        if (itemRows >= MAX_ITEMS) break; itemRows += 1; const item = itemDoc.data() || {};
        ...
        const key = productId ? `p:${productId}` : gtin ? `g:${gtin}` : `d:${normalize(name)}`; const quantity = Math.max(0, toNumber(item.quantidade)); const unitPrice = Math.max(0, toNumber(item.precoUnitario)); const lineTotal = Math.max(0, toNumber(item.valorTotal) || quantity * unitPrice);
        const current = productsMap.get(key) || { key, name, totalSpent: 0, quantity: 0, occurrences: 0, history: [] }; current.totalSpent += lineTotal; current.quantity += quantity; current.occurrences += 1;
        if (name.length > current.name.length) current.name = name;
        if (current.history.length < 40) current.history.push({ dateMs: Number.isFinite(purchaseMs) ? purchaseMs : null, date: Number.isFinite(purchaseMs) ? new Date(purchaseMs).toISOString() : null, unitPrice: money(unitPrice), quantity, lineTotal: money(lineTotal), establishment: String(gasto.estabelecimentoNome || '') });
        productsMap.set(key, current);
      }
    }
  }
  const products = [...productsMap.values()].map((product) => {
    const history = product.history.filter((entry) => entry.unitPrice > 0).sort((a, b) => (a.dateMs ?? Number.MAX_SAFE_INTEGER) - (b.dateMs ?? Number.MAX_SAFE_INTEGER)); const first = history[0] || null; const last = history[history.length - 1] || null;
    ...
    return { name: product.name, totalSpent: money(product.totalSpent), quantity: Math.round(product.quantity * 1000) / 1000, occurrences: product.occurrences, avgUnitPrice: product.quantity > 0 ? money(product.totalSpent / product.quantity) : 0,
      ...
      minUnitPrice: history.length ? Math.min(...history.map((entry) => entry.unitPrice)) : 0, maxUnitPrice: history.length ? Math.max(...history.map((entry) => entry.unitPrice)) : 0,
      history: history.slice(-8).map(({ dateMs: _, ...entry }) => entry) };
  });
  ...
  return { tool: 'listalar_analytics', ok: true, readOnly: true, summary: { purchases, itemRows, uniqueProducts: products.length, totalSpent: money(totalSpent), days: days || null }, productHint: productHint || null,
    matchingProducts: matching.slice(0, 6), topSpend: [...products].sort((a, b) => b.totalSpent - a.totalSpent).slice(0, 6), topFrequency: [...products].sort((a, b) => b.occurrences - a.occurrences).slice(0, 6),
    ...
    priceDecreases: products.filter((product) => product.occurrences >= 2 && product.priceChange < 0).sort((a, b) => a.priceChangePct - b.priceChangePct).slice(0, 6), mostExpensive: [...products].sort((a, b) => b.maxUnitPrice - a.maxUnitPrice).slice(0, 6), interpretationHint: normalize(args?.query || userPrompt) };
}
...
exports.askNexusAgent = onCall(
  ...
  async (request) => {
    ...
    const prompt = String(request.data?.prompt || '').trim(); if (!prompt) throw new HttpsError('invalid-argument', 'Informe uma pergunta.');
    ...
    const system = [
      'Você é Nexus, agente técnico central do Giva. Responda em português do Brasil, direto e claro.',
    ...
    return { answer, agentCore: true, version: '1.2', plan: { objective: planResult.objective || '', project: planResult.project || '', entities: planResult.entities || [], tools: (planResult.tools || []).map((tool) => tool.name), deterministic: Boolean(planResult.deterministic), plannerFallback: Boolean(planResult.plannerFallback) }, toolsUsed: evidence.map((item) => ({ name: item.tool, ok: Boolean(item.ok) })), readOnly: true };
  }
);