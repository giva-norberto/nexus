// Nexus session context v3.1.
// O frontend envia pergunta + histórico. Interpretação de projeto, intenção e ferramentas fica no backend.
(() => {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    window.sendMsg = async (event) => {
      ...
      try {
        ...
        if (data.found) {
          const prefix = data.exact
            ? 'Sim. Confirmei no Firestore'
            : 'Sim. Encontrei uma memória correspondente no Firestore';
          window.nexusAddMsg?.(
            'assistant',
            `${prefix}: [${data.type || 'memória'}] ${data.project ? `${data.project} — ` : ''}${data.text || previous}`
          );
        } else {
          window.nexusAddMsg?.(
            'assistant',
            'Não. Verifiquei o Firestore e não encontrei essa informação salva. Se quiser persistir, use “salve esta informação”.'
          );
        }
      } catch (error) {
        console.error(error);
        window.nexusAddMsg?.(
          'assistant',
          'Não consegui verificar o Firestore agora. Não vou afirmar que a informação está salva sem confirmação real.'
        );
      }
    };
  });
